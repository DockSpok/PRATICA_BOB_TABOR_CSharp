...
namespace EnumsAndSwitch
{
    class Program
    {
        ...
        {
            List<Todo> tarefasDaLista = new List<Todo>()
            {
            ...
            ExemploPraImprimir(tarefasDaLista);
            Console.ReadLine();
        }

        private static void ExemploPraImprimir(List<Todo> tarefas)
        {
            foreach (var todo in tarefas)
            {
                switch (todo.Status)
                {
                ...
                Console.WriteLine("Atividade: " + todo.Description + " : STATUS: " + todo.Status + " horas estimadas: " + todo.EstimatedHours);
            }
        }
    }
